...
(let ((pub-key "{:s}")
      ...
      (pub-exp "{:s}")
      (sig "{:s}"))
  (expect "Signature must be valid" true (rsa.verify-pkcs1-v15 pub-key key-len pub-exp sig "{:s}"))
  (expect "Signature is invalid" false (rsa.verify-pkcs1-v15 pub-key key-len pub-exp sig "{:s}"))
)
...
for s_len in (1024, 2048, 3072, 4096):
    for exponent in (5,17,257,65537):
        gen_sign_data(s_len, exponent, True)
